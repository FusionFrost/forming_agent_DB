...
size_t pdf2txt::FindStringInBuffer(char* buffer, char* search, size_t buffersize)
{
	char* buffer0 = buffer;
...
//Otherwise return -1:
float pdf2txt::ExtractNumber(const char* search, int lastcharoffset)
{
...
bool pdf2txt::seen2(const char* search, char* recent)
{
...
void pdf2txt::ProcessOutput(FILE* file, char* output, size_t len)
{
...
int pdf2txt::Convert(char *pathPdf,char *pathTxt)
{
	//Discard existing output:
	FILE* fileo = fopen(pathTxt, "w");
	if (fileo) fclose(fileo);
	fileo = fopen(pathTxt, "a");

	//Open the PDF source file:
	FILE* filei = fopen(pathPdf, "rb");
